...
// An Emerald's method returns a value if its last sentence is an assignment or
// a method call. If method's last sentence is like that, we return its type
// in $$, otherwise we return NULL.
method_code : 
	separator { $$ = NULL; } 
	| assignment
...
// In $$ we return a pointer to method's argument type.
method_call_argument : 	
	expression
...
// While loop - semantic verifications:
...
// If construction - semantic verifications: 
// expression must return a boolean.
if_construction : 
...
// Each literal returns its corresponding type in the symbols' table.
literal : 
